...
<template>
  <div class="wxc-tab-page"
    ...
    <div class="tab-title-list"
         :style="{ backgroundColor: tabStyles.bgColor, height: (tabStyles.height + (isIPhoneX ? 0 : 0))+'px',paddingBottom:isIPhoneX?'0':'0'}">
      <div class="title-item"
           v-for="(v,index) in tabTitles"
           :key="index"
           :ref="'wxc-tab-title-'+index"
           @click="setPage(index,v.url)"
           :style="{borderLeftWidth:index<=0?'0px':'0px', width: tabStyles.width +'px', height: tabStyles.height +'px',backgroundColor: currentPage == index ? tabStyles.activeBgColor : tabStyles.bgColor }"
           :accessible="true"
           :aria-label="`${v.title?v.title:'标签'+index}`">

        <image resize="contain" :src="currentPage == index ? v.activeIcon : v.icon"
               v-if="titleType === 'icon' && !titleUseSlot"
               :style="{ width: tabStyles.iconWidth + 'px', height:tabStyles.iconHeight+'px', marginTop:tabStyles.iconMarginTop+'px', marginBottom:tabStyles.iconMarginBottom+'px'}"></image>
